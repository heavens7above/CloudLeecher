...
import { Play, Pause, Trash2, File, CheckCircle, AlertCircle, Clock, Download, RefreshCw } from 'lucide-react';
...
function TaskCard({ task }) {
    ...
    const getStatusIcon = (status) => {
        switch (status) {
            ...
            case 'moving': return <RefreshCw size={16} className="animate-spin text-blue-400" />;
        ...
        <Card className="hover:border-emerald-500/30 transition-colors overflow-hidden">
            <CardContent className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                <div className={`p-3 rounded-lg bg-white/5 border border-white/5 ${getStatusColor(task.status)} shrink-0`}>
                    {task.status === 'active' ? <LoaderIcon /> : getStatusIcon(task.status)}
                </div>
