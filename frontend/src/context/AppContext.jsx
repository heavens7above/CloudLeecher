...
export function AppProvider({ children }) {
    ...
    useEffect(() => {
        ...
        setApiUrl(apiUrl);
        // If we have both, check connection automatically?
        // Better to wait for user to click "Connect" or check if already checking.
    ...
    useEffect(() => {
        ...
        const interval = setInterval(async () => {
            try {
                ...
                // Don't disconnect immediately on one failure, but maybe after a few?
            }
        }, 3000);
    ...
    const setConnectionInfo = (url, key) => {
        setApiUrlState(url);
        setApiKeyState(key);
        // Effects will trigger checkConnection
    };
        ...
        for (let i = 0; i < retries; i++) {
            try {
                ...
                if (err.response && err.response.status === 401) {
                    addToast("Unauthorized: Invalid API Key", "error");
    ...
    const refreshStatus = async () => {
        try {
            ...
            if (error.response && error.response.status === 401) {
                setIsConnected(false); // Force disconnect on auth error during polling
            }
            throw error;
        }
    };
        ...
        setTasks(currentTasks => {
            ...
            for (let i = 0; i < newTasks.length; i++) {
                ...
                if (backendTask) {
                    ...
                    // Backend sends 'moving' and 'saved' statuses via active list now
                    ...
                    // For 'moving' or 'saved' tasks, maintain name if file path is just root dir
                    if (newStatus === 'moving' || newStatus === 'saved') {
                         // The backend sends the root folder name as file path for moving tasks
                    }
    ...
    const addMagnet = async (magnet) => {
        ...
        try {
            ...
            if (!res || !res.gid) throw new Error("Invalid backend response");
            ...
            addToast(err.response?.data?.error || "Failed to add magnet link", "error");
            throw err;
        }
    };
    ...
    const addTorrentFile = async (base64Content) => {
        ...
        if (hasActiveDownload) {
            addToast("Download in progress. Please wait.", "warning");
            return null;
        }
        ...
        try {
            ...
            addToast(err.response?.data?.error || "Failed to add torrent file", "error");
            throw err;
        }
    };
    ...
    const clearHistory = () => {
        ...
        tasksToRemove.forEach(t => {
            ...
            TorrentAPI.remove(t.gid).catch(err => console.error("Failed to clear task:", t.gid, err));
        });
    };

    return (
        <AppContext.Provider value={{
            apiUrl, apiKey, setConnectionInfo,
